...
Production의 `catalog-publisher-cli`는 먼저 두 도시 META를 `attribute_not_exists(pk)`
조건의 transaction으로 예약해 같은 Version 재작성을 막고, Place Projection을 25개
단위 BatchWrite한다. Unprocessed Item은 제한된 지수형 재시도로 처리하며, 두 도시의
`CURRENT` Item은 `expectedPreviousVersion` 조건을 가진 하나의 TransactWrite에서 함께
갱신한다. 런타임 Lambda IAM은 GetItem/Query만 사용하고, 게시 권한은 보호된 GitHub
Deploy Role의 Workflow에만 둔다.
