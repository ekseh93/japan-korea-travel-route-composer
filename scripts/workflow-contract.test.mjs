...
test("deployment workflows require reviewed inputs and protected environments", async () => {
  ...
  assert.match(deploy, /trips:compose/);
  assert.match(deploy, /grep -q '.*dayPlans.*'/);
  ...
  assert.match(plan, /manage_lambda_reserved_concurrency=false/);
  assert.match(deploy, /manage_lambda_reserved_concurrency=false/);
});
...
test("AWS-capable workflows require OIDC and protected fork guards", async () => {
  ...
  const reconcile = await readFile(new URL("scripts/reconcile-production-state.sh", root), "utf8");
  ...
  assert.match(reconcile, /untaint_if_managed/);
  assert.match(reconcile, /lambda get-policy/);
  assert.match(reconcile, /AllowHttpApiInvoke/);
  const deploy = await workflow("deploy-production.yml");
