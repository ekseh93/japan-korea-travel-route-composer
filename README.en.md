...
> Protected Production Build Gate `31925333862` failed because the workflow omitted the catalog root; the fix is pending re-verification and no AWS deployment stage was reached
>
...
| Tests and builds                                     | LUN-001~014 Gate format, lint, typecheck, 67 Vitest tests, 4 smoke contract tests, 4 release contract tests, 5 workflow contract tests, 3 Terraform contract tests, 4 browser E2E tests, build, catalog:validate, catalog:build, frozen install, and dependency audit run; local Production Catalog validate/build passed, protected Build Gate `31925333862` failed on the omitted catalog root and is being fixed, Terraform fmt/validate, TFLint, and Trivy passed in the preceding GitHub CI; Bootstrap Apply partially ran, deployment smoke not run |
